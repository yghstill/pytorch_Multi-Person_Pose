...
#    peaks = zip(T.nonzero(peaks_binary)[0],T.nonzero(peaks_binary)[0])
    ...
    peaks_with_score = [x + (map_ori[x[1],x[0]],) for x in peaks_list]
    #id = range(peak_counter, peak_counter + len(peaks)) #py2
    id = range(peak_counter, peak_counter + len(peaks_list))
    peaks_with_score_and_id = [peaks_with_score[i] + (id[i],) for i in range(len(id))]

    all_peaks.append(peaks_with_score_and_id)
    peak_counter += len(peaks_list)
